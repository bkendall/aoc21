...
func main() {
	...
	for _, l := range inputStrings {
		...
		for _, v := range outputs {
			str = fmt.Sprintf("%s%d", str, m[sortString(v)])
		}
		val := strToInt(str)
	...
	fmt.Printf("Total number of desired values: %d\n", sum)
}

var numbersByLength = map[int]int{
	2: 1,
	3: 7,
	4: 4,
	7: 8,
...
func process(in map[string]int) map[string]int {
	res := map[string]int{}

	// Find 1, 4, 7, and 8.
	for l, num := range numbersByLength {
		for k := range in {
			if len(k) == l {
				res[k] = num
			}
	...
	// fmt.Printf("map so far: %+v\n", in)

	// Let's find 0, 6, and 9 first.
	candidates := []string{}
	for k := range in {
		if len(k) == 6 {
	...
	// 6 is the only character that doesn't include 1.
	one, ok := strForNum(res, 1)
	if !ok {
		logFatal("Could not find expected number 1.")
	}
	remove := ""
	for _, c := range candidates {
		if !containsAll(c, one) {
			res[c] = 6
			remove = c
		}
	}
	candidates = dropFromArr(candidates, remove)
	// 9 includes 4.
	four, ok := strForNum(res, 4)
	if !ok {
		logFatal("Could not find expected number 4.")
	}
	for _, c := range candidates {
		if containsAll(c, four) {
			res[c] = 9
			remove = c
		}
	}
	candidates = dropFromArr(candidates, remove)
	// The remaining candidate is 0.
	res[candidates[0]] = 0

	// Now, let's do 2, 3, and 5.
	candidates = []string{}
	for k := range in {
		if len(k) == 5 {
			candidates = append(candidates, k)
	...
	// 3 is the only one that will contain 7.
	seven, ok := strForNum(res, 7)
	if !ok {
		logFatal("Could not find expected number 7.")
	}
	for _, c := range candidates {
		if containsAll(c, seven) {
			res[c] = 3
			remove = c
		}
	}
	candidates = dropFromArr(candidates, remove)
	// 2 and 5 are hard.
	// If I take the string for 6 and remove the chars from 5, I'll be left with 1 char.
	// If I remove the chars from 2, I'll be left with 2 chars.
	six, ok := strForNum(res, 6)
	if !ok {
		logFatal("Could not find expected number 6.")
	}
	switch diff := numCharsDiff(six, candidates[0]); diff {
	case 1:
		res[candidates[0]] = 5
		res[candidates[1]] = 2
	default:
...
func numCharsDiff(a, b string) int {
	m := map[rune]int{}
	for _, r := range a {
		m[r]++
	}
...
const input = `gbcead cfgaeb beadf adb egafd fbeac dbfegca fdaceb dbfc bd | dfcb gedaf dcfb bcfdea
